...
export const PASSKEY_PROGRAM_DEFINITIONS: Record<PasskeyProgramMode, PasskeyProgramDefinition> = {
  ...
  site_passkey_active: {
    ...
    scope: 'shipping_4_2',
    description:
      'Fully operational site passkey with WebAuthn registration and authentication runtime.',
  },
  site_passkey_future_rp: {
    ...
    scope: 'post_4_2',
    description:
      'Reserved for advanced relying-party flows such as cross-device signing and passkey sync.',
  },
};

export const PASSKEY_PROGRAM_SCOPES = {
  shipping_4_1: Object.values(PASSKEY_PROGRAM_DEFINITIONS).filter(
    (item) => item.scope === 'shipping_4_1'
  ),
  shipping_4_2: Object.values(PASSKEY_PROGRAM_DEFINITIONS).filter(
    (item) => item.scope === 'shipping_4_2'
  ),

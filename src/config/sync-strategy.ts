...
export const AEGIS_SYNC_TRANSPORTS: Record<AegisSyncTransportKey, AegisSyncTransportDefinition> = {
  qr_transfer: {
    ...
    descriptionDefault: "Device-to-device transfer stays offline and uses encrypted QR payloads plus a one-time transfer code.",
    trustBoundaryKey: "syncTransportQrBoundary",
    trustBoundaryDefault: "Trust boundary: source device, receiver device, short-lived transfer code.",
  },
  encrypted_backup: {
    ...
    descriptionDefault: "Portable encrypted vault export for restore, migration, and disaster recovery.",
  ...
  plaintext_export: {
    ...
    descriptionDefault: "CSV or JSON export remains a migration-only tool and should stay disabled by default.",
    trustBoundaryKey: "syncTransportPlaintextBoundary",
    trustBoundaryDefault: "Trust boundary: user-managed file handling outside encrypted vault protections.",
  },
  optional_encrypted_sync: {
    ...
    descriptionDefault: "Reserved transport for future end-to-end encrypted multi-device sync, separate from offline-first core flows.",
    trustBoundaryKey: "syncTransportCloudBoundary",
    trustBoundaryDefault: "Trust boundary: encrypted payloads only; remote relay never becomes a plaintext authority.",
  },
};
...
export const AEGIS_SYNC_MODES: Record<AegisSyncModeKey, AegisSyncModeDefinition> = {
  offline_first: {
    ...
    descriptionDefault: "Aegis 4.1 uses local vault storage as the source of truth. Transfer flows stay explicit, user-initiated, and encrypted.",
  ...
  optional_encrypted_sync: {
    ...
    descriptionDefault: "Future encrypted sync remains an opt-in layer that complements, not replaces, offline-first vault ownership.",
...
export const AEGIS_SYNC_CONFLICT_RULES: AegisSyncConflictRuleDefinition[] = [
  {
    ...
    descriptionDefault: "Incoming data never silently replaces the active vault. The local vault remains the main source of truth.",
  },
  {
    ...
    descriptionDefault: "Cross-device data is merged only through explicit import, restore, or future approved sync flows.",
  },
  {
    ...
    descriptionDefault: "Recovery or backup restore must remain a visible user decision, not a background overwrite.",
  },
];

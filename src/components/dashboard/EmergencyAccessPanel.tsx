...
const statusClassName = (status: EmergencyAccessRequest["status"]) => {
  if (status === "granted") return "bg-emerald-500/15 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-100";
  if (status === "approved") return "bg-sky-500/15 text-sky-700 dark:bg-sky-500/20 dark:text-sky-100";
  if (status === "pending") return "bg-amber-500/15 text-amber-700 dark:bg-amber-500/20 dark:text-amber-100";
  if (status === "rejected" || status === "revoked") return "bg-red-500/15 text-red-700 dark:bg-red-500/20 dark:text-red-200";
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <label className="settings-card-item rounded-2xl p-3 flex items-center justify-between gap-3 text-[11px]">
          ...
          <input
            ...
            onChange={(event) => onUpdatePolicy({ default_wait_hours: Number(event.target.value || 48) })}
        ...
        <label className="settings-card-item rounded-2xl p-3 flex items-center justify-between gap-3 text-[11px]">
          ...
          <input
            ...
            onChange={(event) => onUpdatePolicy({ grant_ttl_hours: Number(event.target.value || 24) })}
            className="settings-inline-input w-20 rounded-lg px-2 py-1 text-right"
          />
        </label>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="settings-subpanel rounded-2xl border p-5 shadow-inner">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <select
              value={permission}
              onChange={(event) => setPermission(event.target.value === "full_access" ? "full_access" : "read_only")}
              className="entry-field rounded-lg py-2 px-3 text-sm font-medium outline-none"
            >
            ...
            <input
              ...
              max={720}
              value={waitHours}
              onChange={(event) => setWaitHours(Number(event.target.value || policy.default_wait_hours))}
                ...
                <div key={contact.id} className="settings-card-item rounded-xl p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-semibold text-[var(--color-deep-navy)] dark:text-white">{contact.name}</div>
            ...
            <div className="space-y-2">
              {pendingRequests.length === 0 ? (
                ...
                pendingRequests.slice(0, 6).map((request) => (
                  <div key={request.id} className="rounded-xl border border-black/5 bg-white/70 p-3 dark:border-white/10 dark:bg-white/5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="text-xs font-semibold">{contactsById[request.contact_id]?.email || request.contact_id}</div>
                      <span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-widest ${statusClassName(request.status)}`}>
                        {t(`emergencyAccessStatus.${request.status}`)}
                      </span>
                    </div>
                    <div className="mt-1 text-[11px] opacity-70">{t("emergencyAccessUnlockAt")}: {new Date(request.unlock_at).toLocaleString()}</div>
            ...
            <div className="space-y-2">
              {activeGrants.length === 0 ? (
                ...
                activeGrants.slice(0, 4).map((request) => (
                  <div key={request.id} className="rounded-xl border border-emerald-500/20 bg-emerald-500/8 p-3">
                    <div className="text-xs font-semibold">{contactsById[request.contact_id]?.email || request.contact_id}</div>
                    <div className="mt-1 text-[11px] opacity-70">
                      {t("emergencyAccessExpiresAt")}: {request.expires_at ? new Date(request.expires_at).toLocaleString() : "-"}
                    </div>
        ...
        <div className="space-y-2">
          {auditEvents.length === 0 ? (
            ...
            auditEvents.slice(0, 6).map((event) => (
              <div key={event.id} className="rounded-xl border border-black/5 bg-white/70 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs font-semibold">{t(`emergencyAccessAuditType.${event.type}`)}</div>
                  <div className="text-[10px] opacity-60">{new Date(event.at).toLocaleString()}</div>
                </div>

...
export const SyncRelayControl: React.FC = () => {
  ...
  const regenerateSession = () => {
    if (!window.confirm(t('syncRegenerateConfirm', 'Warning: Regenerating the session ID will disconnect other devices. Continue?'))) return;
    const newSid = crypto.randomUUID();
    setSessionId(newSid);
    SecureAppSettings.setSyncRelaySessionId(newSid);
      ...
      <div className="settings-subpanel p-5 rounded-3xl border shadow-inner transition-all duration-300">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className={`p-3 rounded-2xl ${enabled ? 'bg-emerald-500/10 text-emerald-600' : 'bg-gray-500/10 text-gray-500'}`}>
            ...
            <div>
              ...
              <p className="text-xs opacity-70 leading-relaxed max-w-sm mt-1">
                {t('syncRelayDesc', 'Synchronize your encrypted vault across devices using the secure Aegis Relay service.')}
              </p>
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="p-3 rounded-2xl bg-indigo-500/10 text-indigo-600 mb-3 group-hover:scale-110 transition-transform">
                {isSyncing ? <RefreshCw className="w-6 h-6 animate-spin" /> : <ArrowUp className="w-6 h-6" />}
              </div>
              <span className="text-sm font-bold text-[var(--color-deep-navy)] dark:text-white">{t('syncPushAction', 'Push to Cloud')}</span>
              <span className="text-[10px] opacity-60 mt-1 text-center">{t('syncPushDesc', 'Encrypt and upload local changes')}</span>
            </button>
            ...
            >
              ...
              <div className="p-3 rounded-2xl bg-amber-500/10 text-amber-600 mb-3 group-hover:scale-110 transition-transform">
                {isSyncing ? <RefreshCw className="w-6 h-6 animate-spin" /> : <ArrowDown className="w-6 h-6" />}
              </div>
              <span className="text-sm font-bold text-[var(--color-deep-navy)] dark:text-white">{t('syncPullAction', 'Pull & Merge')}</span>
              <span className="text-[10px] opacity-60 mt-1 text-center">{t('syncPullDesc', 'Download and merge remote changes')}</span>
            </button>
          </div>
          ...
          <div className="settings-card-surface-muted rounded-3xl p-5 border shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-widest opacity-60">{t('syncSessionTitle', 'Sync Session')}</span>
              </div>
            ...
            {showConfig && (
              <div className="mt-5 pt-5 border-t border-black/5 dark:border-white/10 space-y-4 animate-in slide-in-from-top-2 duration-300">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest opacity-60 px-1 dark:text-white">{t('syncRelayUrlLabel', 'Relay URL')}</label>
                  <input 
                    type="text" 
                    value={relayUrl}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest opacity-60 px-1 dark:text-white">{t('syncRelayApiKeyLabel', 'Relay API Key')}</label>
                  <input 
                    type="password" 
                    value={relayApiKey}
          ...
          <div className="v5-sync-security-notice p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-900/40 border border-amber-500/20 dark:border-amber-700/50 flex items-start gap-3">
            ...
            <p className="v5-sync-security-text text-[12px] font-bold leading-relaxed text-amber-950 dark:text-amber-100">
              {t('syncSecurityNotice', 'All data is end-to-end encrypted using your device secret and master password. The relay server never sees your decrypted records.')}
            </p>
          </div>
        </div>
      )}

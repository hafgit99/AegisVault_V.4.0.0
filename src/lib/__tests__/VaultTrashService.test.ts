  ...
  return {
    ...
    mockDb: {
      ...
      delete: vi.fn(async (s: string, id: number) => {
        if (s === 'attachments') attStore.delete(id);
...
function createMockSQLite(entries: any[] = []) {
  ...
  return {
    ...
    mockDb: {
      ...
      deleteAttachment: vi.fn((id: number) => {
        attStore.delete(id);
      }),
      deletePassword: vi.fn((id: number) => {
        store.delete(id);
      }),
...
describe('VaultTrashService', () => {
  ...
  describe('deletePermanently', () => {
    it('deletes entry and attachments from IDB', async () => {
      const { mockDb, store, attStore } = createMockIDB([
        { id: 1, title: 'T', attachments: [{ id: 10 }, { id: 20 }] },
      ]);

...
const releaseTrustSnapshotSourcePath = path.join(repoRoot, "src", "generated", "release-trust-snapshot.ts");
...
function summarizeDeviceMatrixEntries(entries) {
  ...
  for (const entry of list) {
    ...
    if (passkeyRecoveryGroup && Object.values(passkeyRecoveryGroup).some((value) => value === "planned")) {
  ...
  ? {
      ...
      checks: toCheckResult(qualityChecklistTemplate.checks || [], (check) => {
        ...
        if (check.id === "import_export_regression") {
          return { status: summary.importExportRegression?.success ? "passed" : "missing_or_failed" };
        }
        if (check.id === "security_regression") {
          return { status: summary.securityRegression?.success ? "passed" : "missing_or_failed" };
        }
        if (check.id === "e2e") {
          return { status: summary.e2e ? (summary.e2e.success ? "passed" : "failed") : "optional_missing" };
        }
        if (check.id === "extension_builds") {
          const ok = summary.extensionBuilds.chrome.exists && summary.extensionBuilds.firefox.exists;
  ...
  ? {
      ...
      checks: toCheckResult(releaseManifestTemplate.checks || [], (check) => {
        ...
        if (check.id === "provenance") {
          return { status: fs.existsSync(releaseProvenancePath) ? "passed" : "manual_or_release_only" };
        }
        return { status: "unknown" };
      }),
    }
  ...
  ? {
      ...
      checklist: toCheckResult(androidReleaseReadinessTemplate.checklist || [], (check) => {
        if (check.id === "unit_tests") {
          return { status: hasPassLine(androidReleaseReadinessSource, "npm test -- --runInBand") ? "passed" : "missing_or_failed" };
        }
        if (check.id === "typescript") {
          return { status: hasPassLine(androidReleaseReadinessSource, "npx tsc --noEmit") ? "passed" : "missing_or_failed" };
        }
        if (check.id === "assemble_release") {
          return { status: hasPassLine(androidReleaseReadinessSource, ":app:assembleRelease") ? "passed" : "missing_or_failed" };
        }
        if (check.id === "real_device_install") {
          return { status: /Real device install via `adb install -r`: PASS/i.test(androidReleaseReadinessSource || "") ? "passed" : "missing_or_failed" };
        }
  ...
  ? {
      ...
      checklist: toCheckResult(androidProductionChecklistTemplate.checklist || [], (check) => {
        if (check.id === "controlled_beta_readiness") {
          return {
            status:
              extractAndroidExecutiveStatus(androidReleaseReadinessSource) === "ready_for_controlled_beta_release"
                ? "passed"
                : "missing_or_failed",
          };
        }
        ...
        if (check.id === "translation_polish") {
          return { status: fs.existsSync(androidTranslationPolishEvidencePath) ? "passed" : "planned_translation_polish" };
        }
...
const evidenceGaps = {
  generatedAt: summary.generatedAt,
  gaps: [
    ...(qualityChecklist?.checks || []).filter((check) => check.status !== "passed").map((check) => ({
      id: check.id,
      mode: "quality",
      owner: check.owner,
      required: Boolean(check.required),
    ...
    ...(releaseEvidenceManifest?.checks || []).filter((check) => check.status !== "passed").map((check) => ({
      id: check.id,
      mode: "release",
      owner: check.owner,
      required: Boolean(check.required),
    ...
    ...(androidReleaseReadiness?.checklist || []).filter((check) => check.status !== "passed").map((check) => ({
      id: check.id,
      mode: "android_release_readiness",
      owner: check.owner,
      required: Boolean(check.required),
    ...
    ...(androidProductionCandidateChecklist?.checklist || []).filter((check) => check.status !== "passed").map((check) => ({
      id: check.id,
      mode: "android_production_candidate",
      owner: check.owner,
      required: Boolean(check.required),
...
const lines = [
  ...
  androidReleaseReadiness
    ? `Status: ${androidReleaseReadiness.executiveStatus}, Device Matrix: ${
        androidReleaseReadiness.checklist.find((check) => check.id === "device_matrix")?.status || "unknown"
      }`
  ...
  androidProductionCandidateChecklist
    ? `Checklist Items: ${androidProductionCandidateChecklist.checklist.length}, Open Follow-ups: ${
        androidProductionCandidateChecklist.checklist.filter((check) => check.status !== "passed").length
      }`
...
if (releaseEvidenceManifest) {
  fs.writeFileSync(releaseManifestOutputPath, JSON.stringify(releaseEvidenceManifest, null, 2), "utf8");
}
if (androidDeviceMatrix) {
  fs.writeFileSync(androidDeviceMatrixOutputPath, JSON.stringify(androidDeviceMatrix, null, 2), "utf8");
}
if (androidReleaseReadiness) {
  fs.writeFileSync(androidReleaseReadinessOutputPath, JSON.stringify(androidReleaseReadiness, null, 2), "utf8");
}
...
export const RELEASE_TRUST_SNAPSHOT: ReleaseTrustSnapshot = ${JSON.stringify(
  {
    ...
    auditReadyDocs: [
      { id: "external_audit_prep", path: "guvenlik/EXTERNAL_AUDIT_PREP.md", exists: fs.existsSync(path.join(repoRoot, "guvenlik", "EXTERNAL_AUDIT_PREP.md")) },
      { id: "threat_model", path: "guvenlik/THREAT_MODEL.md", exists: fs.existsSync(path.join(repoRoot, "guvenlik", "THREAT_MODEL.md")) },
      { id: "security_whitepaper", path: "guvenlik/SECURITY_WHITEPAPER.md", exists: fs.existsSync(path.join(repoRoot, "guvenlik", "SECURITY_WHITEPAPER.md")) },
      { id: "evidence_ownership", path: "ci-artifacts/evidence-ownership.md", exists: fs.existsSync(path.join(repoRoot, "ci-artifacts", "evidence-ownership.md")) },
    ],

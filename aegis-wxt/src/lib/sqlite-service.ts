  ...
  async init(masterKey: string): Promise<void> {
    ...
    try {
      ...
      if (isOpfsSupported && sqlite3.opfs) {
        console.log(
          '[Aegis WXT] OPFS (Origin Private File System) is supported. Using native file IO.'
        );
        vfsName = 'opfs';
      } else {
        console.warn(
          '[Aegis WXT] OPFS not fully supported (Safari/Older Firefox). Falling back to IndexedDB virtual file system (kvvfs).'
        );
        vfsName = 'kvvfs';
      }
      ...
      console.log(
        '[Aegis WXT] SQLCipher engine successfully unlocked and connected via ' + vfsName
      );
    } catch (error) {

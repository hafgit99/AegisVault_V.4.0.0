...
test.describe('P2P QR Sync Flows', () => {
  ...
  test('Should open QRExporter from Data Management settings', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Cancel Export').or(page.locator('text=Devam Et İptal'))
    ).toBeVisible();
  ...
  test('Should open QRScanner from Data Management settings and handle missing camera gracefully', async ({
    page,
  }) => {

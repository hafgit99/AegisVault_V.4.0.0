  ...
  const steps = [
    {
      ...
      content:
        'Aegis Vault WXT sayesinde şifrelerinizi kopyalamanıza gerek kalmaz. Girdiğiniz web sitelerinde (örn. Netflix, Google) otomatik doldurma aktif olur.',
      icon: <Sparkles className="w-6 h-6 text-white" />,
    },
    {
      title: 'Güvenli Otomatik Doldurma',
      content:
        "Şifre alanına tıkladığınızda 'Aegis' ikonunu göreceksiniz. Vault kasanız açıksa (Unlocked), şifreleriniz sıfır bilgi mimarisiyle anında doldurulur.",
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-4">
        <div className="w-[450px] relative pointer-events-auto">
          ...
          <div className="bg-[rgba(255,255,255,0.9)] backdrop-blur-xl border border-white/50 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            <button
              onClick={closeTour}
              className="absolute top-4 right-4 p-2 text-gray-400 hover:text-red-500 transition-colors bg-black/5 hover:bg-red-50 rounded-full"
            >
              <X className="w-4 h-4" />
            </button>
              ...
              <div className="flex items-center justify-between w-full mt-6 pt-6 border-t border-black/5">
                <div className="flex gap-1.5">
                  {steps.map((_, i) => (
                    <div
                      key={i}
                      className={`h-1.5 rounded-full transition-all duration-300 ${step === i ? 'w-6 bg-[var(--color-sage-green)]' : 'w-1.5 bg-black/10'}`}
                    />
                  ))}
                </div>

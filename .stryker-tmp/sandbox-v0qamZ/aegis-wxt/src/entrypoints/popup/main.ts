...
const buildPopupI18n = (language: 'tr' | 'en'): PopupI18n => {
  ...
  return {
    ...
    bridgePairHint: isTurkishLocale
      ? 'Masaustu uygulamasinda onay penceresi acilacak.'
      : 'A confirmation dialog will open in the desktop app.',
    ...
    bridgePairingModeSigned: isTurkishLocale
      ? 'Kalici imzali eslesme'
...
let popupLanguage: 'tr' | 'en' = normalizeUiLanguage(
...
const formatBridgeError = (code: string) => {
  ...
  const map: Record<string, string> = {
    FORBIDDEN_EXTENSION_ID:
      popupLanguage === 'tr'
        ? 'Bu eklenti kimligi masaustu tarafinda izinli degil.'
        : 'This extension ID is not allowlisted on desktop.',
    PAIRING_REJECTED:
      popupLanguage === 'tr'
        ? 'Eslestirme istegi masaustunde reddedildi.'
        : 'Pairing request was rejected on desktop.',
    NATIVE_HOST_UNAVAILABLE:
      popupLanguage === 'tr'
        ? 'Native host kullanilamiyor veya kayitli degil.'
        : 'Native host is unavailable or not registered.',
    LOOPBACK_FALLBACK_UNAVAILABLE:
      popupLanguage === 'tr'
        ? 'Native host kullanilamiyor ve loopback fallback devre disi. Masaustu uygulamayi yeniden baslatip eslestirmeyi tekrar deneyin.'
        : 'Native host is unavailable and loopback fallback is disabled. Restart the desktop app and try pairing again.',
    INVALID_PAIRING_SECRET:
      popupLanguage === 'tr'
        ? 'Olusturulan eslestirme sirri gecersiz.'
        : 'Generated pairing secret is invalid.',
    NATIVE_BRIDGE_TIMEOUT:
      popupLanguage === 'tr'
        ? 'Masaustu koprusu zaman asimina ugradi.'
        : 'Desktop bridge timed out.',
    NATIVE_BRIDGE_EOF:
      popupLanguage === 'tr'
        ? 'Masaustu koprusu beklenmedik sekilde kapandi.'
        : 'Desktop bridge closed unexpectedly.',
    NATIVE_BRIDGE_SECRET_MISSING:
      popupLanguage === 'tr'
        ? 'Masaustu koprusunde eslestirme sirri bulunamadi.'
        : 'Pairing secret is missing on desktop bridge.',
    CLIENT_PUBLIC_KEY_REQUIRED:
      popupLanguage === 'tr'
        ? 'Kalici eslestirme icin istemci anahtari gerekli.'
        : 'A client key is required for persistent pairing.',
    CLIENT_KEY_ID_MISMATCH:
      popupLanguage === 'tr'
        ? 'Istemci anahtar kimligi uyusmuyor.'
        : 'Client key identity mismatch.',
    INVALID_NATIVE_BRIDGE_SIGNATURE:
      popupLanguage === 'tr'
...
const bootPopup = async () => {
  try {
    ...
    popupTheme =
      themeRes?.theme === 'dark' ||
      (themeRes?.theme !== 'light' && window.matchMedia('(prefers-color-scheme: dark)').matches)
        ? 'dark'
        : 'light';
  } catch {
    popupLanguage = normalizeUiLanguage(
      typeof navigator !== 'undefined' ? navigator.language : 'en'
    );
    POPUP_I18N = buildPopupI18n(popupLanguage);
  ...
  const renderBridgeStatus = async () => {
    ...
    try {
      ...
      const sourceLabel =
        source === 'runtime'
          ? POPUP_I18N.bridgeSourceRuntime
          : source === 'build'
            ? POPUP_I18N.bridgeSourceBuild
      ...
      if (metaEl) {
        ...
        if (paired && desktopPairing?.lastApprovedAt) {
          detailLines.push(
            `<div>${POPUP_I18N.bridgeLastApproved}: ${desktopPairing.lastApprovedAt}</div>`
          );
        }
        if (paired && desktopPairing?.deviceFingerprint) {
          detailLines.push(
            `<div>${POPUP_I18N.bridgeDeviceFingerprint}: ${desktopPairing.deviceFingerprint}</div>`
          );
        }
        if (paired && desktopPairing?.pairingMode) {
          detailLines.push(
            `<div>${POPUP_I18N.bridgePairingMode}: ${desktopPairing.pairingMode === 'signed-p256-v1' ? POPUP_I18N.bridgePairingModeSigned : POPUP_I18N.bridgePairingModeLegacy}</div>`
          );
        }
        if (paired && desktopPairing?.clientKeyId) {
          detailLines.push(
            `<div>${POPUP_I18N.bridgeClientKey}: ${desktopPairing.clientKeyId}</div>`
          );
        }
        if (riskFlags.length > 0) {
          detailLines.push(
            `<div style="color:${popupTheme === 'dark' ? '#f87171' : '#b45309'};font-weight:600;">${POPUP_I18N.bridgeRiskPrefix}: ${riskFlags.join(', ')}</div>`
          );
        }
  ...
  const wireBridgeActions = () => {
    ...
    pairBtn?.addEventListener('click', async () => {
      ...
      try {
        ...
        const metaEl = document.getElementById('bridge-meta');
        if (metaEl)
          metaEl.textContent = formatBridgeError(
            err instanceof Error ? err.message : 'PAIRING_FAILED'
          );
      } finally {
  ...
  const loadPopup = async () => {
    ...
    try {
      ...
      if (!vaultStatus?.isUnlocked) {
        if (statusEl) {
          statusEl.innerText = POPUP_I18N.vaultLocked;
      ...
      if (!response?.success || passwords.length === 0) {
        if (statusEl) {
          statusEl.innerText = POPUP_I18N.vaultEmpty;
          statusEl.style.color = '#f59e0b';
        }
        if (containerEl) {
      ...
      if (statusEl) {
        statusEl.innerText = POPUP_I18N.vaultOpen;
      ...
      matches.forEach((p: PopupCredential) => {
        ...
        card.addEventListener('click', async () => {
          ...
          if (fillBtn) {
            fillBtn.innerText = `✓ ${POPUP_I18N.filled}`;
            fillBtn.style.color = '#22c55e';
          }

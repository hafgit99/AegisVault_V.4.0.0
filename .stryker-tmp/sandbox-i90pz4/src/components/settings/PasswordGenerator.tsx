    ...
    <div className="password-generator-surface border border-[var(--color-sage-green)]/30 bg-gradient-to-br from-[var(--color-sage-green)]/5 to-transparent rounded-3xl p-6 shadow-sm relative overflow-hidden flex flex-col gap-6">
      <div className="absolute bottom-0 left-0 right-0 h-32 opacity-20 pointer-events-none overflow-hidden">
        <div
          ...
          style={{ transform: `translateX(-${genEntropy % 50}%)` }}
        >
          <svg
            className={`w-full h-full fill-[var(--color-sage-green)] ${genEntropy > 60 ? 'animate-wave-fast' : 'animate-wave'}`}
      ...
      <div className="flex items-center justify-between relative z-10">
        ...
        <span
          className={`text-xs font-bold px-3 py-1 rounded-md ${genEntropy > 80 ? 'bg-[var(--color-sage-green)]/20 text-[var(--color-sage-green)]' : 'bg-red-500/10 text-red-500'}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        <div className="flex flex-col gap-2">
          <label className="text-sm font-semibold opacity-70">
            {t('lengthLabel')}: <span className="text-[var(--color-sage-green)]">{genLength}</span>
          </label>
          <input
            type="range"
            min="8"
            ...
            className="w-full accent-[var(--color-sage-green)]"
          />
        </div>
        <div className="password-generator-toggle flex items-center justify-between md:justify-center gap-3 bg-white/40 p-3 rounded-xl border border-white">
          ...
          <input
            ...
            className="accent-[var(--color-sage-green)] w-5 h-5 rounded cursor-pointer"
          />
        </div>
        <div className="password-generator-toggle flex items-center justify-between md:justify-center gap-3 bg-white/40 p-3 rounded-xl border border-white">

      ...
      candidate.bindings && typeof candidate.bindings === 'object'
        ? Object.fromEntries(
            Object.entries(candidate.bindings).map(([k, v]) => [
              k,
              {
                credentialId: (v as any).credentialId || '',
                encryptedPayload: (v as any).encryptedPayload || '',
                prfSalt: (v as any).prfSalt || '',
                eventLog: Array.isArray((v as any).eventLog) ? (v as any).eventLog : [],
                meta: {
                  ...(v as any).meta,
                  version: (v as unknown as Record<string, any>)?.meta?.version || 1,
                },
              },
            ])
          )

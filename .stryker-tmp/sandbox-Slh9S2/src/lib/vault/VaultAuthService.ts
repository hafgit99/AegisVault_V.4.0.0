...
import type { StoredCredential } from '../../vaultService';
...
export class VaultAuthService {
  ...
  static async migrateCredentialToArgon2(
    ...
    return this.createAuthCredential(password, params);
  }
  ...
  static async sha256Hex(value: string): Promise<string> {
    const input = new TextEncoder().encode(value);
    const digest = await window.crypto.subtle.digest('SHA-256', toBufferSource(input));

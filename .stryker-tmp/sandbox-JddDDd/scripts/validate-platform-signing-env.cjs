...
function main() {
  if (!requirePlatformSigning) {
    console.log("[release:platform-signing-env] platform signing not required in this environment.");
    return;
  }
  ...
  if (process.platform === "win32") {
    ...
    if (!process.env.APPLE_APP_SPECIFIC_PASSWORD) errors.push("APPLE_APP_SPECIFIC_PASSWORD_MISSING");
    if (!process.env.APPLE_TEAM_ID) errors.push("APPLE_TEAM_ID_MISSING");
  }

  if (errors.length > 0) {
    console.error(`[release:platform-signing-env] missing required environment: ${errors.join(", ")}`);
    process.exit(1);
  }

    ...
    it('2. Migrasyon Hatası: SQLite devre dışı kalır ve IDB fallback başlar', async () => {
        ...
        mockIDB.get.mockImplementation((store: string, key: string) => {
            if (store === 'vault_metadata' && key === 'auth_credential')
                return Promise.resolve({ id: 'auth_credential', credential: { verificationHash: 'h', salt: 'bHRvYmU=', scheme: 'argon2id-v1' } });
        ...
        vi.spyOn(vaultService as any, 'deriveMasterKey').mockResolvedValue('salt_b64');
        vi.spyOn(vaultService as any, 'verifyPassword').mockResolvedValue(true);

...
export function SharedSpacesModal({
  ...
  return (
    <div className="fixed inset-0 z-[210] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-[var(--color-deep-navy)]/50 backdrop-blur-sm" onClick={onClose} />
      <div className="settings-drawer-surface relative z-10 flex h-[min(94vh,980px)] w-full max-w-6xl flex-col overflow-hidden rounded-[2.25rem] border border-white/20 shadow-2xl lg:flex-row">
        ...
        <aside className="overflow-y-auto border-b border-white/10 bg-gradient-to-br from-[#789072] via-[#6f866b] to-[#5f745e] p-5 text-white lg:w-[290px] lg:shrink-0 lg:border-b-0 lg:border-r">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-sm text-sm leading-7 text-white/78">{t("sharedSpacesDesc")}</p>
            </div>
            <button onClick={onClose} className="rounded-full p-2 text-white/70 transition-colors hover:bg-white/10 hover:text-white">
              <X className="h-5 w-5" />
            </button>
          </div>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="text-right text-xs text-white/75">
                    ...
                    <div>{t("sharedSpacesDefaultRole", { role: t(`sharedSpacesRole.${space.default_role}`) })}</div>
                  </div>
                </div>
              </button>
            ))}
        ...
        <section className="flex min-h-0 min-w-0 flex-1 flex-col">
          <div className="border-b border-white/8 px-5 py-4 lg:px-6">
            ...
            {initialSpaceId && editingSpaceId === initialSpaceId ? (
              <div className={`mt-4 rounded-[1.5rem] border border-[var(--color-sage-green)]/20 bg-[var(--color-sage-green)]/10 px-5 py-4 text-sm text-[var(--color-deep-navy)] dark:border-[var(--color-sage-green)]/25 dark:bg-[var(--color-sage-green)]/10 dark:text-white ${pulseFocus ? "shadow-[0_0_0_10px_rgba(117,141,114,0.08)]" : ""}`}>
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--color-sage-green)]">
                  {focusContext === "audit" ? t("sharedSpacesFocusAuditTitle") : t("sharedSpacesFocusIssueTitle")}
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-2">
                ...
                <input
                  ...
                  onChange={(event) => setDraftSpace((current) => ({ ...current, name: event.target.value }))}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <label className="settings-card-surface-muted flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-inner">
                <input
                  ...
                  onChange={(event) =>
                    setDraftSpace((current) => ({ ...current, allow_export: event.target.checked }))
                  }
                />
              ...
              <label className="settings-card-surface-muted flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-inner">
                <input
                  ...
                  onChange={(event) =>
                    setDraftSpace((current) => ({ ...current, require_review: event.target.checked }))
                  }
                />
                <span className="text-sm text-[var(--color-deep-navy)] dark:text-white">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-xs text-[var(--color-deep-navy)]/60 dark:text-white/60">
                            {member.email || t(`sharedSpacesStatus.${member.status}`)} | {t(`sharedSpacesRole.${member.role}`)}
                          </div>

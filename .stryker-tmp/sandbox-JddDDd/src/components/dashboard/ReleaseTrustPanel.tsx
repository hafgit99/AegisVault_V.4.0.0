...
export function ReleaseTrustPanel({
  ...
  const packageReadiness = new Map(
    summary.auditReadyPackages.map((pkg) => {
      const resolvedCount = pkg.checklistKeys.filter((key) => Boolean(resolvedChecklistStatus[key])).length;
  ...
  const phaseNineReady =
    packageStatusCounts.ready === summary.auditReadyPackages.length && summary.auditReadyPackages.length > 0;
  const riskTone =
    ...
    <div className="settings-panel rounded-3xl border p-6 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="rounded-2xl bg-[var(--color-sage-green)]/10 p-3 text-[var(--color-sage-green)]">
            {summary.openGapCount === 0 ? <ShieldCheck className="h-5 w-5" /> : <ShieldAlert className="h-5 w-5" />}
          </div>
        ...
        <div className="text-right">
          ...
          <div className="mt-1 text-3xl font-black text-[var(--color-deep-navy)]">{summary.score}</div>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="settings-card-surface-muted rounded-2xl border px-4 py-3">
          ...
          <div className="mt-1 text-base font-semibold text-[var(--color-deep-navy)]">{summary.openGapCount}</div>
        </div>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {summary.checks.map((check) => (
          ...
          >
            <div>
              <div className="font-semibold text-[var(--color-deep-navy)]">{t(`releaseTrustCheck.${check.id}`)}</div>
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border p-4">
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          {summary.ownerBreakdown.map((owner) => (
            <div key={owner.owner} className="settings-card-surface rounded-2xl border px-4 py-3 text-sm">
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border px-4 py-3 text-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          >
            {phaseNineReady ? t("releaseTrustPhaseNineComplete") : t("releaseTrustPhaseNineInProgress")}
          </div>
        </div>
        <div className="settings-section-copy mt-2">
          {phaseNineReady ? t("releaseTrustPhaseNineDescComplete") : t("releaseTrustPhaseNineDescInProgress")}
        </div>
      </div>
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border p-4">
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          {summary.ownerActions.map((action) => (
            <div key={action.id} className="settings-card-surface rounded-2xl border px-4 py-3 text-sm">
              <div className="flex items-center justify-between gap-3">
                <div className="font-semibold text-[var(--color-deep-navy)]">{t(action.titleKey)}</div>
              ...
              <div className="mt-2 text-[11px] font-medium text-[var(--color-deep-navy)]/55">{action.targetPath}</div>
              <div className="mt-3 space-y-2">
                ...
                {action.packageLinkIds.map((linkId) => {
                  ...
                  if (!link) return null;
                  return (
                    <div key={link.id} className="settings-card-surface rounded-xl border px-3 py-2 text-xs">
                      <div className="font-semibold text-[var(--color-deep-navy)]">{t(link.labelKey)}</div>
                      <div className="mt-1 text-[11px] text-[var(--color-deep-navy)]/60">{link.path}</div>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border p-4">
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          {summary.auditReadyPackages.map((pkg) => {
            ...
            return (
            <div key={pkg.id} className="settings-card-surface rounded-2xl border px-4 py-3 text-sm">
              <div className="flex items-center justify-between gap-3">
                <div className="font-semibold text-[var(--color-deep-navy)]">{t(pkg.titleKey)}</div>
                <div className="settings-badge-muted rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.18em]">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <div className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${readinessTone}`}>
              ...
              <div className="mt-3 space-y-2">
                ...
                {pkg.checklistKeys.map((checkKey) => {
                  ...
                  return (
                    <div key={checkKey} className="settings-card-surface rounded-xl border px-3 py-2 text-xs">
                      <div className="flex items-center justify-between gap-3">
                        <div className="text-[var(--color-deep-navy)]/75 leading-relaxed">{t(checkKey)}</div>
                        <button
                          type="button"
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border p-4">
        ...
        <div className="mt-3 space-y-3">
          {summary.auditReadyLinks.map((item) => (
            <div key={item.id} className="settings-card-surface rounded-2xl border px-4 py-3 text-sm">
      ...
      <div className="settings-card-surface-muted mt-5 rounded-2xl border p-4">
        ...
        <div className="mt-3 space-y-3">
          {historyItems.length === 0 ? (
            ...
            <div key={event.id} className="settings-card-surface rounded-2xl border px-4 py-3 text-sm">
                <div className="font-semibold text-[var(--color-deep-navy)]">
                  {t(`releaseTrustHistoryAction.${event.action}`)}
                </div>
                <div className="mt-1 text-xs text-[var(--color-deep-navy)]/60">{event.title || event.targetId}</div>
